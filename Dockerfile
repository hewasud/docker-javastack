...
https://download.oracle.com/otn-pub/java/jdk/8u202-b08/1961070e4c9b4e26a04e7f5a083f551e/jdk-8u202-linux-x64.rpm \
	...
	&& rm -f jdk-8u202-linux-x64.rpm \
 	...
 	&& ln -sf apache-maven-3.6.0 latest\
 	...
 	&& cd /opt/tomcat \
 	&& curl -LO --silent http://mirror.its.dal.ca/apache/tomcat/tomcat-8/v8.5.37/bin/apache-tomcat-8.5.37.tar.gz \
 	&& tar zxf apache-tomcat-8.5.37.tar.gz \
 	&& ln -sf apache-tomcat-8.5.37 latest \
 	&& rm -f apache-tomcat-8.5.37.tar.gz \
 	&& echo "org.apache.catalina.webresources.Cache.level=SEVERE" >> /opt/tomcat/latest/conf/logging.properties \
  	...
  	&& mkdir -p /opt/intellij/ideaUI-2018.3 \
